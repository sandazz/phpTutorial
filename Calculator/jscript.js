var inputstring = "";
var inp, i, postindex = -1,
    stackindex = -1;
var stackOperators = new Array(30),
    post = new Array(30),
    answerstack = new Array(30),
    index = -1;
var disp;
onload = main;
...
function evaluate() {
    for (i = 0; post[i] != "D"; i++) {
        ...
        if ((c == '+') || (c == '-') || (c == '*') || (c == '/')) {
            switch (post[i]) {
                case '+':
                    answerstack[index - 1] = answerstack[index - 1] + answerstack[index];
                    break;
                case '-':
                    answerstack[index - 1] = answerstack[index - 1] - answerstack[index];
                    break;
